...
export class UserEditProfileComponent implements OnInit, OnDestroy {
  ...
  getCountryList() {
    const countryListSubscribe = this._commonService.countryList().subscribe((data: any) => {
      if (data.result == 1 && data.data.length > 0) {
        ...
        this.countryList = data.data.map((country: any) => ({ value: country.id, text: country.name }));
      } else {
        this.countryList = [
          { value: 1, text: 'India' },
          { value: 2, text: 'USA' },
  ...
  getCityList(countryId: any) {
    countryId = countryId.target ? countryId.target.value : countryId;
    const cityListSubscribe = this._commonService.cityList(countryId).subscribe((data: any) => {
      if (data.result == 1 && data.data.length > 0) {
        // Map backend data to dropdown format
        this.cityList = data.data.map((city: any) => ({ value: city.id, text: city.name }));
      } else {
        this.cityList = [
          { value: 1, text: 'Ahmedabad' },
          { value: 2, text: 'Mumbai' },
